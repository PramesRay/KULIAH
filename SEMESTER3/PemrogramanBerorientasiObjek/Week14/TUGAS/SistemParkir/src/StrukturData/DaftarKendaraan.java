...
public class DaftarKendaraan {
    ...
    public String traversal(){
        String hasil = "";
        Kendaraan bantu = this.firstKendaraan;
        
        if(this.firstKendaraan == null){
            System.out.println("Tidak ada kendaraan yang parkir");
        } else {
            int total = 0, no = 1;
            hasil += "\t\t\t\t\t\t\t\t\t\t\tRekapitulasi Biaya Kendaraan Mall Jatos\n\n";
            hasil += String.format("%-5s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s | %-15s%-10s%-5s%-15s%-15s\n", 
                            "NO",
                            "Jenis",
                            "Plat No", 
                            "Brand", 
